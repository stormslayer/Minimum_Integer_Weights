...
# used in shapley_gauss -- this is the Shapley+ measured used in our empirical work -- its set to 10k
its = 1000
Enoise = .165   # this is derived from all cases post-WW2 for use in Shapley_gauss 
R = 0.5          # discount for opposition parties for policy loss implemented by a winning coalition 
...
def coal_weighted_median(proposed_coal):
    # determine median policy outcomes for a coalition 
    cmed = []
    pol_med = -1

    # add policy to list weighted by the number of seats in parliament
    for i in proposed_coal:
        for j in range(0, int(temp_data[i][1])):
            cmed.append(temp_data[i][7])
...
def slack_find(tslack):
    # this finds the value of slack for each coalition using medians 
    # there are, however, many options for slack -- using raw seats, salience weighted, using a pivotality measure, etc.
    # the unidimensional policy position here is a weighted median of the unidimensional estimate from CHES 

    # depends on global temp_data and global parliament_median
    ...
    for i in tslack.keys():
        ...
        # coalitions form at median
        tslack[i] = 1.0
        for j in i:
            # distance from coalition policy median reduces avail slack; price paid in reversion from parliament median added back in
            tslack[i] = tslack[i] - abs(coal_median(i) - temp_data[j][7]) + R*abs(parliament_median - temp_data[j][7])
...
def leverage_find(tleverage):    
    ...
    for i in tleverage.keys():
        for j in i:
            ...
            # this need not be a new coalition that includes ALL of the other members -- it must be individually rational
            # keep best alternative slack if such a MWC exists
            best_alt_slack = 0.0
            for k in tleverage.keys():
                if k != i:
                    if set(tcoal).intersection(set(k)) != set() and (j not in k):        # check for null set: set() 
...
for z in case_data_input:
    ...
    # calculate global median of the entire parliament for use in functions above
    parliament_median = coal_weighted_median(unity)
